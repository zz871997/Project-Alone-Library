...
public class ThongKeController {
	...
	private String[][] getDataTKSachFromDB(int indexOfCb) {
	...
	private void setActionForThongKeSachView(String[][] data) {
		JButton btnPrint  = thongKeView.getBtnPrint();
		JButton btnCancel = thongKeView.getBtnCancel();
	...
	private void inThongKe(String[][] data) {
		...
		int select = fileChooser.showSaveDialog(this.thongKeView);
		String saveFilePath = "";
	...
	private void saveThongKeExcelTo(String path, String data[][]) {
		try {
			...
			cell = row.createCell(0, CellType.STRING);
			cell.setCellValue(thongKeInformation.getLbTitle().getText().toString().toUpperCase());
			...
			cell.setCellValue(cbTKSach.getSelectedItem().toString().toUpperCase());
			cell.setCellStyle(createStyleForTableTitle(workbook));
			cell = row.createCell(2, CellType.STRING);
	...
	private void huy() {
		this.thongKeView.setVisible(false);
	}
}
